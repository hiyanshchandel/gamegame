...
try:
    ...
    background_img = pygame.transform.scale(background_img, (2000, 600))
except:
    background_img = None
...
for _ in range(len(obstacles)):
    # Random speed between 2 and 4
    speed = random.uniform(2, 4)
    obstacle_speeds.append(speed)
    
    # Random initial direction (-1 for left, 1 for right)
    direction = random.choice([-1, 1])
    obstacle_directions.append(direction)
    
    # Define movement range for each obstacle (100-300 pixels from starting position)
...
while True:
    ...
    if not game_over:
        ...
        # Scroll the background when Mario reaches the middle
        if mario_x > 400:
            scroll_offset -= mario_speed_x
            mario_x = 400

        # Update and check collision with obstacles
        mario_rect = pygame.Rect(mario_x, mario_y, 50, 50)
        for i, obstacle in enumerate(obstacles):
            ...
            # Check if obstacle has reached its movement limits
            if new_x <= obstacle_movement_ranges[i]['start']:
                obstacle_directions[i] = 1  # Change direction to right
                new_x = obstacle_movement_ranges[i]['start']
            elif new_x >= obstacle_movement_ranges[i]['end']:
                obstacle_directions[i] = -1  # Change direction to left
                new_x = obstacle_movement_ranges[i]['end']
            
            # Update obstacle position
            obstacles[i].x = new_x

        # Update score based on total distance
