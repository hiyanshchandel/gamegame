...
platforms = [
    ...
    pygame.Rect(2000, 300, 200, 20),
    pygame.Rect(2500, 250, 200, 20),
]
...
obstacle_speeds = [2, -3, 1]
obstacle_directions = [1, -1, 1]

# Initialize score and track Mario's last position
score = 0
last_position_x = mario_x  # Track Mario's last position to calculate distance traveled
...
def reset_game():
    ...
    global mario_x, mario_y, mario_velocity_y, scroll_offset, on_ground, score, last_position_x
    mario_x, mario_y = mario_start_x, mario_start_y
    mario_velocity_y = 0
    scroll_offset = 0
    on_ground = False
    score = 0
    last_position_x = mario_x
...
while True:
    ...
    if not game_over:
        ...
        # Update score based on horizontal movement
        distance_traveled = mario_x - last_position_x
        if distance_traveled > 0:
            score += distance_traveled // 10  # Adjust the divisor for score scaling
        last_position_x = mario_x  # Update last position
